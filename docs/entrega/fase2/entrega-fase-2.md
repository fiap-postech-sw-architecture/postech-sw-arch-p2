...
Cada requisito da fase 2 ([gap analysis](https://github.com/fiap-postech-sw-architecture/postech-sw-arch-p2/blob/main/docs/requisitos/fase2/gap-analysis-fase-2.md)) está mapeado para o PR que o implementou, para a evidência principal no código e para o ponto do [roteiro do vídeo](https://github.com/fiap-postech-sw-architecture/postech-sw-arch-p2/blob/main/docs/entrega/fase2/roteiro-video.md) que o demonstra.

### Requisitos funcionais

| ID | Requisito | PR | Evidência (arquivo / teste chave) | Demonstração no vídeo |
|---|---|---|---|---|
| RF-020 | Abertura de OS com cliente, veículo, serviços e peças, retornando id único | [#15](https://github.com/fiap-postech-sw-architecture/postech-sw-arch-p2/pull/15) | `CriarOrdemDTO` com `servicos`/`pecas` (`src/ordem_servico/aplicacao/dtos.py`) e montagem única de itens em `use_cases.py`; e2e `tests/integracao/test_api_e2e.py::TestCriacaoOsComItens` | Bloco 4 — `POST /ordens-de-servico/` com itens → 201 + `id` |
| RF-021 | Consulta de status no vocabulário do enunciado | [#14](https://github.com/fiap-postech-sw-architecture/postech-sw-arch-p2/pull/14) | `situacao_de` em `src/ordem_servico/aplicacao/situacoes.py` + campo `situacao` nos 3 schemas de resposta; `tests/unitarios/ordem_servico/test_presenters.py` | Bloco 4 — `GET /ordens-de-servico/{id}` e acompanhamento público com `situacao` |
| RF-022 | Endpoint externo de aprovação/recusa de orçamento | [#16](https://github.com/fiap-postech-sw-architecture/postech-sw-arch-p2/pull/16) | Rota `POST /api/v1/publico/ordens-de-servico/{ordem_id}/decisao-orcamento` em `src/compartilhado/interfaces/router_publico.py`; use case `DecidirOrcamento`; [ADR-021](https://github.com/fiap-postech-sw-architecture/postech-sw-arch-p2/blob/main/docs/arquitetura/adr/fase2/021-aprovacao-externa-orcamento.md) | Bloco 4 — aprovar e recusar com assinatura HMAC (`X-Webhook-Signature` + `X-Webhook-Timestamp`, TD-027) |
...
O backlog de dívida técnica é mantido como um ledger versionado em [`docs/tech-debt/README.md`](https://github.com/fiap-postech-sw-architecture/postech-sw-arch-p2/blob/main/docs/tech-debt/README.md), com itens classificados e rastreados desde a fase 1. O ledger registra hoje **28 itens resolvidos** e **6 abertos** (todos deliberados e justificados, sem impacto de produção no caminho suportado; os achados da [auditoria pré-entrega](https://github.com/fiap-postech-sw-architecture/postech-sw-arch-p2/blob/main/docs/tech-debt/auditoria-pre-entrega-fase2.md) foram endereçados por completo). Fora do escopo exigido pela fase 2, o grupo amortizou parte expressiva desse backlog — incluindo um débito tático de DDD herdado da fase 1 — atacando-o em tiers priorizados ([plano-ataque.md](https://github.com/fiap-postech-sw-architecture/postech-sw-arch-p2/blob/main/docs/tech-debt/plano-ataque.md)) e mantendo o ledger fiel ao código. Os principais:
...
| TD-022 (métricas do relay — observabilidade) | Pilar de **métricas** no relay: Prometheus no cluster + `MeterProvider` OTel/`PrometheusMetricReader` expondo `/metrics`, opt-in por env; complementa a [ADR-020](https://github.com/fiap-postech-sw-architecture/postech-sw-arch-p2/blob/main/docs/arquitetura/adr/fase2/020-observabilidade-opentelemetry.md) na parte de métricas ([ADR-024](https://github.com/fiap-postech-sw-architecture/postech-sw-arch-p2/blob/main/docs/arquitetura/adr/fase2/024-metricas-prometheus.md)) | [#66](https://github.com/fiap-postech-sw-architecture/postech-sw-arch-p2/pull/66) |
| TD-015 (corrida de migração multi-réplica — infra) | Migração movida do entrypoint do pod para um Job dedicado (`pytstop-migrate`), aplicado antes do rollout com `kubectl wait --for=condition=complete`; `RUN_MIGRATIONS_ON_STARTUP`/`RUN_SEED_ON_STARTUP` passam a `false` no cluster — resolve a corrida com N réplicas | [#64](https://github.com/fiap-postech-sw-architecture/postech-sw-arch-p2/pull/64) |
| TD-011 (DAST automatizado — segurança) | OWASP ZAP baseline automatizado no `full-test-ci` (DAST contra a stack compose), relatório como artefato + alvo `make dast` para paridade local | [#65](https://github.com/fiap-postech-sw-architecture/postech-sw-arch-p2/pull/65) |
| TD-021 (relay HA — fencing de lease) | Fencing de lease na entrega (re-lock `FOR UPDATE SKIP LOCKED` + checagem de status na transação por-linha) torna `replicas>1` seguro sem duplicar entrega, sem mudança de schema | [#66](https://github.com/fiap-postech-sw-architecture/postech-sw-arch-p2/pull/66) |
| TD-023 (rate-limit por cliente real atrás de proxy — segurança/RNF-024) | `ProxyHeadersMiddleware` do uvicorn aplicado quando `TRUSTED_PROXIES` está configurado, reescrevendo `request.client` a partir do `X-Forwarded-For` confiável; default vazio (não confia em XFF, sem spoof) | [#67](https://github.com/fiap-postech-sw-architecture/postech-sw-arch-p2/pull/67) |
| TD-005 (orçamento em coluna Text — domínio) | Coluna `orcamento_json` migrada de `Text` para `jsonb` nativo (migração 004), removendo a camada manual `json.dumps`/`json.loads` no mapping; serialização passa a seguir o padrão de `outbox.payload` | [#68](https://github.com/fiap-postech-sw-architecture/postech-sw-arch-p2/pull/68) |
| TD-007 (contato como primitivo — DDD) | `Contato` Value Object (texto livre validado, `__repr__` PII-safe) substitui o `contato: str` primitivo no agregado `Cliente`, persistindo na mesma coluna via shadow + event listeners (padrão CPF/Placa, sem migração) | [#70](https://github.com/fiap-postech-sw-architecture/postech-sw-arch-p2/pull/70) |
| TD-010 (SonarQube como gate de CI — segurança) | Fechado **por decisão**: SonarQube/SonarCloud não vira gate de CI (repo privado + custo desproporcional ao MVP); a análise estática em CI é CodeQL + ruff + bandit, com SonarQube como scan manual de fechamento | [#71](https://github.com/fiap-postech-sw-architecture/postech-sw-arch-p2/pull/71) |
