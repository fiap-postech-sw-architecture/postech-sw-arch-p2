...
> **Ambiente de demonstração na nuvem:** `CLOUD-URL-FASE-2` — disponível **24/7 durante julho/2026** (horário de Brasília); a banca pode abrir a qualquer hora do mês. A partir de 01/08/2026 o ambiente é destruído para preservar o crédito de estudante e reerguível sob demanda em ~10 min (deploy 100% reproduzível). Custo mantido baixo por _node_ único, exposição só da UI (a API permanece interna) e destruição fora da janela — risco financeiro zero (conta de estudante sem cartão). Este ambiente é um **diferencial**: o aceite do enunciado (vídeo + repositório + IaC do kind) não depende dele.
