...
**Além dos requisitos**: observabilidade com OpenTelemetry — **traces** de FastAPI e SQLAlchemy no Jaeger ([ADR-020](https://github.com/fiap-postech-sw-architecture/postech-sw-arch-p2/blob/main/docs/arquitetura/adr/fase2/020-observabilidade-opentelemetry.md), [PR #22](https://github.com/fiap-postech-sw-architecture/postech-sw-arch-p2/pull/22)) e, depois, **métricas** do relay no Prometheus — profundidade da outbox, idade do pendente mais antigo, DLQ e contadores de entrega/falha/retry, via `MeterProvider` OTel + `PrometheusMetricReader` ([ADR-024](https://github.com/fiap-postech-sw-architecture/postech-sw-arch-p2/blob/main/docs/arquitetura/adr/fase2/024-metricas-prometheus.md), [PR #66](https://github.com/fiap-postech-sw-architecture/postech-sw-arch-p2/pull/66), TD-022); ambos no cluster de demo, demonstrados no bloco 6 do vídeo.

### Qualidade além do escopo — dívida técnica endereçada

O backlog de dívida técnica é mantido como um ledger versionado em [`docs/tech-debt/README.md`](https://github.com/fiap-postech-sw-architecture/postech-sw-arch-p2/blob/main/docs/tech-debt/README.md), com itens classificados e rastreados desde a fase 1. O ledger registra hoje **26 itens resolvidos** e **5 abertos** (todos os 5 abertos são simplificações deliberadas/justificadas, sem risco de produção no caminho suportado; os achados da [auditoria pré-entrega](https://github.com/fiap-postech-sw-architecture/postech-sw-arch-p2/blob/main/docs/tech-debt/auditoria-pre-entrega-fase2.md) foram endereçados por completo numa campanha de tiers priorizados). Fora do escopo exigido pela fase 2, o grupo amortizou parte expressiva desse backlog — incluindo um débito tático de DDD herdado da fase 1 — atacando-o em tiers priorizados ([plano-ataque.md](https://github.com/fiap-postech-sw-architecture/postech-sw-arch-p2/blob/main/docs/tech-debt/plano-ataque.md)) e mantendo o ledger fiel ao código. Os principais:
...
| TD-022 (métricas do relay — observabilidade) | Pilar de **métricas** no relay: Prometheus no cluster + `MeterProvider` OTel/`PrometheusMetricReader` expondo `/metrics` (profundidade da outbox, idade do pendente mais antigo, DLQ, contadores de entrega/falha/retry), opt-in por env; reabre a [ADR-020](https://github.com/fiap-postech-sw-architecture/postech-sw-arch-p2/blob/main/docs/arquitetura/adr/fase2/020-observabilidade-opentelemetry.md) na parte de métricas ([ADR-024](https://github.com/fiap-postech-sw-architecture/postech-sw-arch-p2/blob/main/docs/arquitetura/adr/fase2/024-metricas-prometheus.md)) | [#66](https://github.com/fiap-postech-sw-architecture/postech-sw-arch-p2/pull/66) |
...
| #75 (gates de segurança em CI) | Automação dos scanners que os docs de segurança citavam mas nenhum workflow rodava: **pip-audit** (CVE em deps — pegou e corrigiu 5 CVEs reais em cryptography/starlette), **gitleaks** (segredos) e **trivy** (CVE na imagem) em [`security.yml`](https://github.com/fiap-postech-sw-architecture/postech-sw-arch-p2/blob/main/.github/workflows/security.yml); escopo do **bandit** ampliado (`src ui relay scripts`); **CodeQL** confirmado no default setup do GitHub + `make codeql-quality` local aplicando as supressões de FP que o default setup não permite; **Dependabot** semanal | [#116](https://github.com/fiap-postech-sw-architecture/postech-sw-arch-p2/pull/116) |

A tabela acima destaca os itens de maior valor; o conjunto completo dos **26 resolvidos** (incluindo os TD-024–031 da campanha pós-auditoria e fechamentos da fase 1 como TD-001, TD-012, TD-018 e TD-020) está no [ledger](https://github.com/fiap-postech-sw-architecture/postech-sw-arch-p2/blob/main/docs/tech-debt/README.md#itens-resolvidos-26).
