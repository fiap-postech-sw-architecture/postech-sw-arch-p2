...
- **Estado atual**: inexistente (`ls infra` falha); banco hoje só via compose (`docker-compose.yml:20-50`) e imagem seedada (`db-image/README.md:1-9`).
...
| `db-image/` | Imagem Postgres seedada no GHCR + compose standalone para a banca (`db-image/README.md:1-9,17`) | **Adaptar (escopo reduzido)** | O deploy oficial do banco passa a ser Terraform + pipeline (RNF-021/RNF-022). A imagem seedada continua valiosa para o fast-check local da banca e como seed de dados de demo no cluster local. Manter sincronizada com as migrations para evitar drift; aposentar somente se a banca passar a validar exclusivamente via cluster. |
