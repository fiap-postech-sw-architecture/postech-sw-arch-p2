...
| RNF-024 | Statelessness para escala horizontal: comportamento correto com N réplicas | JWT stateless com denylist compartilhada no Postgres (`src/autenticacao/interfaces/middleware.py:39-47`); rate limiter slowapi in-memory por processo (`src/compartilhado/interfaces/middleware.py:98`); pool SQLAlchemy com defaults sem dimensionamento (`src/compartilhado/infraestrutura/database.py:16`) | Parcial: rate limiter divergia entre réplicas; pool não dimensionado | Storage compartilhado do rate limiter (ex.: Redis) ou documentar limite por-réplica; configurar `pool_size`/`max_overflow`/`pool_pre_ping` validando `max_connections` no pior caso do HPA — RESOLVIDO: rate limiter agora com **storage compartilhado (Redis)** via `storage_uri` (env `RATE_LIMIT_STORAGE_URI`, fallback in-memory), limite correto sob HPA ([ADR-023](../../arquitetura/adr/fase2/023-rate-limiter-storage-compartilhado.md) / [TD-016](../../tech-debt/README.md), PR #62); pool segue nos defaults do SQLAlchemy |
...
- **Estado atual**: JWT já é stateless com denylist de revogação compartilhada no Postgres (`src/autenticacao/interfaces/middleware.py:39-47`; `src/autenticacao/infraestrutura/token_revogado_repository.py`); o **rate limiter** passou a usar **storage compartilhado (Redis)** via `storage_uri` (env `RATE_LIMIT_STORAGE_URI`, com fallback in-memory quando ausente — dev/CI), tornando o limite por IP correto e global sob HPA ([ADR-023](../../arquitetura/adr/fase2/023-rate-limiter-storage-compartilhado.md), [TD-016](../../tech-debt/README.md), PR #62). Dois pontos finos: (a) uma **queda do Redis em runtime degrada para per-réplica** (`in_memory_fallback_enabled=True`) e volta ao compartilhado no retorno — não derruba a API (sem 500); (b) a chave é o IP do *peer* imediato (`get_remote_address`), então o limite **por cliente real atrás de proxy/ingress depende de `X-Forwarded-For` confiável** (+ uvicorn `--proxy-headers`) — sem isso o tráfego externo colapsa num só bucket; rastreado em [TD-023](../../tech-debt/README.md) (não se manifesta no demo ClusterIP/port-forward). Remanescente: o pool de conexões com defaults do SQLAlchemy sem dimensionamento (`src/compartilhado/infraestrutura/database.py:16`).
- **Mudança**: rate limiter com storage compartilhado (Redis) — ENTREGUE; resta configurar `pool_size`/`max_overflow`/`pool_pre_ping` e validar contra `max_connections` do Postgres no pior caso do HPA.
