...
**Veredito final aplicado (fechamento da fase 2)**: `ui/` mantida dev-only sem adaptação — o kanban não exibe OS encerradas no default pós RF-023, gap aceito como [TD-020](../../tech-debt.md); `full-test/` mantido como harness E2E e gerador de carga; `db-image/` rebaixado no README raiz a atalho da fase 1 (imagens GHCR sem RF-020..024), republicação como `-p2` registrada como opcional futuro ([TD-018](../../tech-debt.md)).
