...
- **Aprovação** é admin-only via JWT: `POST /{ordem_id}/aprovacao` (`src/ordem_servico/interfaces/router.py:281-294`, `Depends(exigir_papel("admin"))`), delegando a `AprovarOrcamento` (`src/ordem_servico/aplicacao/use_cases.py:382-415`), que reserva estoque e transita para `EM_EXECUCAO`.
- **Recusa do orçamento inicial não existe**: o que há é o cancelamento genérico admin-only (`router.py:329-343` → `CancelarOrdem`, `use_cases.py:462-509`) e a rejeição do orçamento **complementar** (`router.py:378-391` → `RejeitarOrcamentoComplementar`, `use_cases.py:557-576`), que devolve a OS a `EM_EXECUCAO` mantendo o escopo originalmente aprovado.
