...
- **Migrações no deploy**: pelo **entrypoint existente**. O `entrypoint.sh` já executa `alembic upgrade head` no boot quando `RUN_MIGRATIONS_ON_STARTUP=true` (padrão `false`) — caminho que o `full-test-ci.yml` já exercita no compose; no cluster, a variável entra pelo ConfigMap. As alternativas — Job dedicado pós-apply (mais limpo para produção) e initContainer (duplicaria a lógica do entrypoint) — adicionariam manifests e ordenação no pipeline sem necessidade para o aceite; o menor delta vence, e o Job dedicado fica como evolução se a corrida de migração com múltiplas réplicas se materializar (ver consequências).
...
* Migração via entrypoint tem janela de corrida se múltiplas réplicas partirem simultaneamente com migração pendente; mitigação: rollout inicial com réplica única (o HPA escala depois) e evolução para Job dedicado de migração se a corrida se materializar
