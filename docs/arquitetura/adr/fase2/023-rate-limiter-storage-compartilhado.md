...
* Chave de rate limit pelo IP real do cliente atrás de proxy (**TD-023, fechado — PR #67**): com `TRUSTED_PROXIES` definida o `ProxyHeadersMiddleware` do uvicorn reescreve `request.client` a partir do `X-Forwarded-For` confiável (peer na lista) antes do limiter ler a chave; default vazio não confia em XFF (sem spoof). Ver [dívida técnica](../../../tech-debt/README.md)
