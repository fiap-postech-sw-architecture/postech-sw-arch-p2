...
**Fechamento dos code smells (12/07/2026)** — a fase 3 já tinha zerado o mesmo conjunto de regras no seu próprio scan (PR #6 lá, arquivos byte-idênticos aos da fase 2 na origem); este fechamento aplica o diff equivalente aqui. Container SonarQube recriado do zero (credenciais antigas não recuperadas — sem impacto, é scan local efêmero); os 3 hotspots já tratados (linha acima) foram re-triados idênticos no novo servidor, já que o estado de revisão vive no banco do Sonar, não no código. Distribuição dos 143 code smells por regra:
