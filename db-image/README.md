...
`docker-compose.yml` standalone que o examinador da banca usa para rodar a
stack completa em 1 comando, e (3) o tutorial QUICKSTART.

> **Para o examinador rodar a demo: ler [`QUICKSTART.md`](QUICKSTART.md)**.
...
|---|---|---|
| [`QUICKSTART.md`](QUICKSTART.md) | sim | Tutorial fast-check para a banca (5 min) |
...
[QUICKSTART.md](QUICKSTART.md) para exemplos de leitura.

## Pre-requisitos para rodar `update-ghcr`

| Requisito | Como configurar |
|---|---|
| `docker` (engine + buildx) | parte do setup base do projeto |
| `make reset-db` funcionando | `update-ghcr` reutiliza o seed via reset-db |
...
Todos rodados a partir da raiz do repositorio.
...
| `make ghcr-prune` | Apaga via `gh api` versoes "untagged" dos **3 packages** (mantem somente a tagueada corrente em cada) |
...
1. **`db-image/docker-compose.yml` foi modificado** com a `ENCRYPTION_KEY` rotacionada que cifra o snapshot recem-publicado. **Sem commitar essa mudanca, qualquer um que clonar o repositorio nao vai conseguir abrir os CPF/CNPJ do dump** (decryption error). O target imprime no fim os comandos exatos para commitar.

2. **`.env.dev` local volta para `OLD_KEY`** (via `trap` na saida) -- mas a DB local ficou seedada com `NEW_KEY`. Resultado: listar clientes localmente vai falhar ate voce rodar `make reset-db` (reseed com `OLD_KEY`).

Por que rotacionar a key a cada publicacao: a key cifra CPF/CNPJ no dump seedado e fica commitada em texto plano em `db-image/docker-compose.yml` (dados sao sinteticos, sem PII real). Rotacionar garante que (a) a key publica so vale para o snapshot atual, (b) leak da key historica nao decifra snapshots novos, e (c) o `.env.dev` local fica com key DIFERENTE da publicada, evitando reuso acidental do valor publico em ambiente nao-demo.
...
1. **Makefile** computa `GIT_SHA := $(shell git rev-parse HEAD)` no parse,
   exporta para o `DOCKER_COMPOSE` wrapper como env var.
2. **`docker-compose.yml`** (local) e **build args dos targets GHCR**
   passam `GIT_SHA`/`GIT_DATE` para os Dockerfiles.
...
aparecem como `-dirty`. Commite antes de rodar `update-ghcr` se quiser que
a SHA reflita o estado real.

## Customizacao

Variaveis sobrescriteis na linha de comando. Defaults: `GHCR_REGISTRY=ghcr.io`, `GHCR_USER=jbamaral`, `GHCR_PREFIX=postech-sw-arch-p1` (gera `<prefix>-db`, `<prefix>-app`, `<prefix>-ui`).

```bash
make update-ghcr GHCR_USER=fork-user GHCR_PREFIX=postech-sw-arch-p1
```

Em forks, sobrescrever `GHCR_USER` e fazer `docker login` / `gh auth refresh` com o usuario do fork. O `ghcr-prune` valida que `gh` esta autenticado como `GHCR_USER` antes de rodar -- se divergir, falha rapidamente com mensagem.

## Por que `--provenance=false --sbom=false`?

Por padrao o buildx publica 3 manifests por push (image manifest +
...
para esta finalidade (demo/snapshot interno), nao recomendado para
